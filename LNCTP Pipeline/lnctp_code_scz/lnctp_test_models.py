...
class sparseModel(tf.keras.Model):
    ...
    def __init__(self, embed_nodes, dense_nodes, activation, cell_types = 8, params =34, batch_norm = True):
...
class custom_mask(tf.keras.constraints.Constraint):
    def __init__(self, embed_nodes, cell_types = 8, params =34):
...
class shallowSparseModel(tf.keras.Model):
    #same as other model but with embedding layer nodes for customization
    def __init__(self, embed_nodes, activation, cell_types = 8, params =34,  batch_norm = True):

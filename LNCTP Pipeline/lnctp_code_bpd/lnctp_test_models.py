...
class sparseModel(tf.keras.Model):
    ...
    def __init__(self, embed_nodes, dense_nodes, activation, cell_types = 8, params =35, batch_norm = True):
...
class custom_mask(tf.keras.constraints.Constraint):
    def __init__(self, embed_nodes, cell_types = 8, params =35):
...
class shallowSparseModel(tf.keras.Model):
    #same as other model but with embedding layer nodes for customization
    def __init__(self, embed_nodes, activation, cell_types = 8, params =35,  batch_norm = True):
            ...
            for dense_node in dense_nodes:
                for lr in learning_rates:
                    for batch_size in batch_sizes:
                        ...
                        model = tf.keras.models.load_model('models_saved/' + spec + '_' + str(fold), compile=False)
